...
def jtable_ajax_list(col_obj,url,urlfieldparam,request,excludes=[],includes=[],query={}):
    ...
    if request.is_ajax():
        ...
        acl = get_acl_object(col_obj._meta['cripts_type'])
        ...
        for doc in response['Records']:
            for key, value in doc.items():
                ...
                if key == "campaign":
                    ...
                    srcs = []
                    if col_obj._meta['cripts_type'] == 'ActorIdentifier':
...
def generate_roles_jtable(request, option):
    ...
    jtopts = {
        ...
        'listurl': reverse('cripts.core.views.roles_listing', args=('jtlist',)),
        'deleteurl': reverse('cripts.core.views.roles_listing',
                             args=('jtdelete',)),
...
def generate_sector_jtable(request, option):
    ...
    fields = ['name', 'Dataset', 'EmailAddress',
              'Event', 'Hash', 'Target', 'UserName']
...
def get_role_details(rid, roles, analyst):
    ...
    from cripts.core.forms import RoleSourceEdit
...
def edit_role_name(rid, old_name, name, analyst):
    ...
    if old_name != settings.ADMIN_ROLE and name != settings.ADMIN_ROLE:
        ...
        CRIPTsUser.objects(roles=old_name).update(set__roles__S=name)
        return {'success': True}
    else:
        return {'success': False}
...
def render_role_graph(start_type="roles", start_node=None, expansion_node=None,
    ...
    data = {'children': []}
    url = reverse('cripts.core.views.role_graph')
    ...
    if start_type == "role" or start_type not in ['source', 'user']:
        ...
        for role in roles:
            ...
            users = CRIPTsUser.objects(roles=role.name)
            for user in users:
