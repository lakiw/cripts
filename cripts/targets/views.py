...
from cripts.targets.forms import TargetInfoForm
from cripts.targets.handlers import upsert_target, get_target_details
from cripts.targets.handlers import remove_target
from cripts.targets.handlers import generate_target_jtable, generate_target_csv
...
def target_search(request):
    ...
    return HttpResponseRedirect(reverse('cripts.emails.views.emails_listing') +
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if results['success']:
                ...
                message += '"%s">%s</a></div>' % (reverse('cripts.targets.views.target_info',
                                                          args=[new_email]),
                                                  new_email)
        ...
        if request.is_ajax():
            ...
            return HttpResponseRedirect(reverse('cripts.targets.views.target_info',
                                                args=[email]))
    else:
        return render_to_response("error.html",
    ...
    if request.method == "POST":
        ...
        if not results['success']:
            ...
            return render_to_response("error.html",
                                      {"error" : error },
                                      RequestContext(request))
        return HttpResponseRedirect(reverse('cripts.targets.views.target_details'))
    else:
        return render_to_response("error.html",
                                  {"error" : "Expected POST" },
                                  RequestContext(request))
