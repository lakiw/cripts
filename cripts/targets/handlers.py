...
from cripts.core.user_tools import is_user_subscribed, user_sources
from cripts.core.user_tools import is_user_favorite

from cripts.services.handlers import run_triage
...
def upsert_target(data, analyst):
    ...
    ##TODO: Update this with the real target data when I impliment this
...
def generate_target_jtable(request, option):
    ...
    jtable['toolbar'] = [
        ...
        },
        {
            ...
            'text': "'Refresh'",
            'click': "function () {$.get('"+reverse('cripts.%ss.views.%ss_listing' % (type_,type_))+"', {'refresh': 'yes'}); $('target_listing').jtable('load');}",
        },
        {
            'tooltip': "'Add Target'",
            'text': "'Add Target'",
